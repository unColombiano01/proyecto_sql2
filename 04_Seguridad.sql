...
-- 1. Administrador_Sistema: todos los privilegios (arrodillaos ante el admin).
...
-- 3. Analista_Datos: solo lectura sobre todas las tablas (excepto auditoria).
...
-- 4. Empleado_Inventario: solo puede modificar la tabla productos (stock y ubicacion).
...
-- Usuarios y asignacion de estos (incluye política de contraseñas seguras: expiración y bloqueo)
...
-- Restricciones (tatequietos) y permisos especificados
...
-- 13. Vista que oculta informacion sensible y privada de los clientes (que void)
...
-- 16. Asegurarse de que root no se use desde sitios o lugares desconocidos: Elimina cualquier cuenta root cuyo host no sea localhost.
...
-- 18. Limitar las consultas por hora para el Analista_Datos (pa que no se meta en sitios raros)
...
-- 19. Vista por la sucursalL: cada usuario solo ve las ventas de su sucursal
...
-- 20. auditoria de los intentos de inicio de sesion fallidos (pa que vean hackers)
...
-- Consultas de la verificacion de seguridad
