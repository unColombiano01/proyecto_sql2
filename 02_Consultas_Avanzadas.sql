...
-- 1. los 10 productazos mas vendidos
SELECT
    p.id_producto,
...
-- 2. productos con bajas ventas con el 10% inferior de ventas (candidatos a sacarlos a patadas).
...
-- 3. clientes VIP: los 5 clientes con mayor valor de vida compradora y los pupis del market
SELECT
    c.id_cliente,
...
-- 4. Analisis mensual de ventas: ventas totales agrupadas por mes y año (con crecimiento).
SELECT
...
-- 5. aumento de clientes: numero de nuevos clientes registrados por trimestre.
SELECT
...
-- 6. Tasa repetitiva de compras: porcentaje de personas que dan fuente de ingreso que hayan realizado más de una compra.
...
-- 7. productos comprados juntos seguidos: pares de productos comprados en la misma transacción con posibles intenciones ocultas.
SELECT
...
-- 8. rotacion de inventario: tasa de  paso a paso de stock para cada categoría de productos.
SELECT
...
-- 9. Productos que necesitan full traida: stock actual por debajo del umbral mínimo osea que estan igual que mercado venezolano.
SELECT
    p.id_producto,
    p.sku,
    p.nombre,
...
-- 10. Analisis de carritos botados: clientes que agregaron productos pero no compraron por culpa de sus hijos o por despistados.
SELECT
    c.id_cliente,
...
-- 11. Rendimiento de proveedores: clasificación según cuanto se venden sus productos.
SELECT
    prov.id_proveedor,
    prov.nombre                                     AS proveedor,
    prov.email_contacto,
...
-- 12. Analisis geografico de ventas: ventas agrupadas por ciudad o region del peronsa que da fuente de ingresos a cambio d eun bien.
SELECT
...
-- 13. Ventas por hora del dia: horas pico de compras para mejorar campañas de marketing (y politicas de paso).
SELECT
...
-- 14. Impacto de las promociones a los clients y sus carteras: comparar ventas de un producto antes, durante y después de un descuento.
...
-- 15. Analisis de cohort: retencion (chismoseada) de clientes mes a mes desde su primera compra.
...
-- 17. Tiempo promedio entre compras: tiempo medio que tarda un cliente en volver a comprar (que no se note el hambre del establecimiento).
...
-- 18. Productos mas vistos y los comprados: comparar ranking de compras con precio/stock.
SELECT
    p.id_producto,
    p.nombre,
    RANK() OVER (ORDER BY COALESCE(SUM(dv.cantidad * dv.precio_unitario_congelado), 0) DESC)
...
-- 19. Segmentacion de clientes (RFM): clasificación por recencia, frecuencia y monetario (clasismo disimulado).
...
-- 20. Prediccion de demanda simple: proyectada de ventas del proximo mes por categoría (promedio movil).
