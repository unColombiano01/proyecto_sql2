...
-- 5. trg_log_new_customer_after_insert: registra en auditoria cada nuevo cliente.
...
-- 8. trg_prevent_negative_stock: impide que el stock se actualice a un valor negativo (no hay sistema joven).
...
-- 9. trg_capitalize_nombre_cliente: capitaliza nombre y apellido al insertar un cliente (para mantenerlo bajo vigilanci).
...
-- 10. trg_recalculate_total_venta_on_detalle_change: recalcula el total de la venta (el antiguo era de 120, ahora debes pagar 300).
...
-- 12. trg_prevent_price_zero_or_less: impide que el precio sea cero o negativo (porque si no perdemos plata).
...
-- 17. trg_prevent_self_referral: impide que un cliente se refiera a sí mismo (que se empiece a identificar como el dueño de la tienda usando eso a su favor).
...
-- Verificar los triggers que ya estan hechesitos
