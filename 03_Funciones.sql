...
-- 1. fn_CalcularTotalVenta: calcula el monto total de una venta especifica.
...
-- 2. fn_VerificarDisponibilidadStock: valida si hay stock suficiente para un producto (porque si no no se puede).
...
-- 4. fn_CalcularEdadCliente: calcula la edad de un cliente a partir de su fecha de nacimiento (los menores de 1 año tienen sus compras gratis).
...
-- 5. fn_FormatearNombreCompleto: devuelve el nombre del cliente en un formato estandarizado (y de paso nos guardamos su informacion).
...
-- 6. fn_EsClienteNuevo: VERDADERO si el cliente realizo su primera compra en los últimos 30 días (para que lo tengamos en la mira desde ahora).
...
-- 7. fn_CalcularCostoEnvio: calcula el costo de envio segun el peso (no el de kilos) total de la venta.
...
-- 8. fn_AplicarDescuento: aplica un porcentaje de descuento a un monto dado (para hacer que compren mas).
...
-- 9. fn_ObtenerUltimaFechaCompra: devuelve la fecha de la última compra de un cliente (pa vigilarlo).
...
-- 12. fn_ContarVentasCliente: cuenta el número total de compras realizadas por un cliente (para saber que ponerle para que compre).
...
-- 13. fn_CalcularDiasDesdeUltimaCompra: dias transcurridos desde la última compra de un cliente (para vigilarlo).
...
-- 14. fn_DeterminarEstadoLealtad: asigna estado de lealtad (Bronce, Plata, Oro) según el gasto total (basado de metodos y demas de El Libro Negro de la Manipulación de Masas de Alexander H. Fitz).
...
-- 16. fn_CalcularIVA: calcula el impuesto (IVA) sobre un subtotal (para ganar mas plata).
DROP FUNCTION IF EXISTS fn_CalcularIVA$$
CREATE FUNCTION fn_CalcularIVA(
    p_subtotal     DECIMAL(12,2),
    p_tasa_iva_pct DECIMAL(5,2)   
)
RETURNS DECIMAL(12,2)
DETERMINISTIC
NO SQL
...
-- 18. fn_EstimarFechaEntrega: estima la fecha de entrega según la ubicación del cliente (doxxeo legal).
...
-- 19. fn_ConvertirMoneda: convierte un monto a otra moneda usando una tasa fija (nadie se salva de pagar).
...
-- 20. fn_ValidarComplejidadContrasena: verifica que una contraseña cumpla criterios de seguridad (para usarla luego a nuestro beneficio).
...
-- Prueba de las funciones hechas
...
-- Reporte combinado usando varias de las funciones funciones
SELECT
    c.id_cliente,
