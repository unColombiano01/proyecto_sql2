...
-- Activa el planificador de los eventos (pues es totalmente necesario pa que los eventos se ejecuten).
...
-- 2. evt_cleanup_temp_tables_daily: borra datos temporales/alertas que se resuelven a diario.
...
-- 3. evt_archive_old_logs_monthly: archiva (o elimina) logs de más de 6 meses (apenas tan nuevecitos).
...
-- 4. evt_deactivate_expired_promotions_hourly: desactiva productos sin stock obsoletos (que ya estan aprobados pa vender).
...
-- 5. evt_recalculate_customer_loyalty_tiers_nightly: recalcula el gasto total de cada cliente (o alterarlo pa hacer que pague mas).
...
-- 6. evt_generate_reorder_list_daily: crea la lista de productos a reabastecer (we need more money).
...
-- 8. evt_suspend_inactive_accounts_quarterly: detecta cuentas inactivas (>1 año) (nos destruyen el corazon y nuestras ganancias).
...
-- 11. evt_send_birthday_greetings_daily: genera lista de clientes que cumplen años (hay que ponernos humildes una vez al dia con ellos).
...
-- 13. evt_backup_critical_tables_daily: backup logico de las tablas mas importantes.
...
-- 18. evt_detect_fraudulent_activity_hourly: detecta patrones sospechosos (pedidos cancelados) (no se salvaran de nuestros precios tan facilmente).
...
-- 20. evt_purge_soft_deleted_records_weekly: elimina productos inactivos nunca vendidos (como ella te elimino de su vida).
